...
void TestUatSignature::uat_sig_010_signatureRoundTripsAsFlattenedImage() {
    ...
    const QString pdfPath = writeSampleSignedPdf(
        m_scratch.filePath(QStringLiteral("sig010.pdf")));
    const QString pngPath = writeSamplePng(
        m_scratch.filePath(QStringLiteral("ink.png")));
    ...
    QVERIFY2(found, qPrintable("No flattened signature XObject: "
                               + keys.join(", ")));
