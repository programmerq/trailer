...
QSize fitDetectionSize(const QSize& src) {
    ...
    if (longest > kDetMaxSide) {
        const float scale =
            static_cast<float>(kDetMaxSide) / static_cast<float>(longest);
...
// grid matching (w, h).
std::vector<uint8_t> binarize(const std::vector<float>& probs,
...
QVector<QRect> labelComponents(std::vector<uint8_t>& bin, int w, int h) {
    ...
    std::vector<uint8_t> visited(
        wz * static_cast<size_t>(h), 0);
    for (int y = 0; y < h; ++y) {
        for (int x = 0; x < w; ++x) {
            const size_t i =
                static_cast<size_t>(y) * wz + static_cast<size_t>(x);
            ...
            if (area < kMinBlobArea) continue;
            rects.append(QRect(minX, minY,
...
QRect unscaleAndExpand(const QRect& r, const QSize& detSize,
                       const QSize& origSize) {
    const float sx =
        static_cast<float>(origSize.width()) /
        static_cast<float>(detSize.width());
    const float sy =
        static_cast<float>(origSize.height()) /
        static_cast<float>(detSize.height());
    QRect out(
        static_cast<int>(std::floor(r.x() * sx)),
...
void sortReadingOrder(QVector<OcrEngine::TextBlock>& blocks) {
    ...
    for (const auto& b : blocks) heights.push_back(b.polygon.boundingRect().height());
    std::sort(heights.begin(), heights.end());
    const int medianH = heights[heights.size() / 2];
    const int rowTol = std::max(2, medianH / 2);
    std::sort(blocks.begin(), blocks.end(),
              [rowTol](const OcrEngine::TextBlock& a,
                       const OcrEngine::TextBlock& b) {
        ...
        if (std::abs(ay - by) > rowTol) return ay < by;
...
int recWidthForCrop(int cropW, int cropH) {
    if (cropH <= 0) return kRecMinWidth;
...
std::vector<float> makeRecInput(const QImage& crop, int targetW) {
    ...
    const float scale = static_cast<float>(kRecHeight) /
                        static_cast<float>(srcH);
    const int resizedW = std::clamp(
        static_cast<int>(std::round(srcW * scale)), 1, targetW);

    const QImage resized = crop
        .convertToFormat(QImage::Format_RGB888)
        .scaled(resizedW, kRecHeight,
                Qt::IgnoreAspectRatio, Qt::SmoothTransformation);

    const size_t plane =
        static_cast<size_t>(kRecHeight) * static_cast<size_t>(targetW);
    ...
    for (int y = 0; y < kRecHeight; ++y) {
        ...
        for (int x = 0; x < resizedW; ++x) {
            ...
            const float b = scan[x * 3 + 2] / 255.0f;
            const size_t idx =
                static_cast<size_t>(y) * static_cast<size_t>(targetW) +
                static_cast<size_t>(x);
...
QString ctcDecode(const std::vector<float>& logits, int T, int C,
                  const QStringList& dict, float* confOut) {
    QString out;
    int prev = -1;
    ...
    for (int t = 0; t < T; ++t) {
        ...
        if (dictIndex < 0 || dictIndex >= dict.size()) continue;
        out.append(dict.at(dictIndex));
        logConfSum += static_cast<double>(
            std::log(std::max(bestProb, 1e-6f)));
        ++kept;
    }
    if (confOut) {
        *confOut = kept > 0
            ? static_cast<float>(std::exp(logConfSum / kept))
            : 0.0f;
    }
    return out;
}
...
OcrEngine::OcrEngine(ModelRegistry* registry, QObject* parent)
    ...
    connect(m_registry, &ModelRegistry::downloadProgress, this,
            [this](ModelId id, qint64 r, qint64 t) {
                if (id == ModelId::PpOcrDetector ||
                    id == ModelId::PpOcrRecognizerLatin) {
...
QVector<QRect> OcrEngine::detectBoxes(const QImage& source) {
    ...
    if (probs.size() < needed) return {};

    auto bin = binarize(probs, detSize.width(), detSize.height(),
                        kDetThreshold);

    // Stash the detection mask (at original-image size) for debug.
    QImage mask(detSize.width(), detSize.height(),
                QImage::Format_Grayscale8);
    for (int y = 0; y < detSize.height(); ++y) {
        uchar* scan = mask.scanLine(y);
        for (int x = 0; x < detSize.width(); ++x) {
            scan[x] = bin[static_cast<size_t>(y) * detWz +
                          static_cast<size_t>(x)] ? 255 : 0;
        }
    }
    m_lastMask = mask.scaled(source.size(),
                             Qt::IgnoreAspectRatio,
                             Qt::SmoothTransformation);

    QVector<QRect> detRects =
        labelComponents(bin, detSize.width(), detSize.height());
    QVector<QRect> out;
...
QString OcrEngine::recognizeBox(const QImage& crop, float* confOut) {
    ...
    if (m_dict.isEmpty() && !loadDictionary()) return {};
    ...
    if (shape.size() != 3 || shape[0] != 1) return {};
    const int T = static_cast<int>(shape[1]);
    const int C = static_cast<int>(shape[2]);
    if (C != m_dict.size() + 1) {
        qWarning() << "OcrEngine: recogniser class count" << C
                   << "does not match dictionary size" << m_dict.size();
        return {};
    }
...
QVector<OcrEngine::TextBlock> OcrEngine::recognize(const QImage& source) {
    if (source.isNull() || !isModelReady()) return {};

    if (!m_detector) {
        m_detector = OnnxSession::fromFile(
            m_registry->localPath(ModelId::PpOcrDetector));
    }
    if (!m_recognizer) {
        m_recognizer = OnnxSession::fromFile(
            m_registry->localPath(ModelId::PpOcrRecognizerLatin));
    }
    if (!m_detector || !m_recognizer) return {};
    if (m_dict.isEmpty() && !loadDictionary()) return {};
    ...
    for (const QRect& box : boxes) {
        const QRect clipped =
            box.intersected(QRect(QPoint(), canvas.size()));
        ...
        TextBlock tb;
        tb.polygon = QPolygon(QVector<QPoint>{
            clipped.topLeft(), clipped.topRight(),
