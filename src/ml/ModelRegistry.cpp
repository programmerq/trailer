...
bool verifyModelHash(const QString& path, const QString& expectedSha256) {
    if (expectedSha256.isEmpty()) return QFile::exists(path);
    QFile f(path);
...
bool ModelRegistry::ensureAvailable(ModelId id) {
    ...
    if (verifyModelHash(dest, it->sha256)) {
        ...
        QMetaObject::invokeMethod(this, [this, id, dest]() {
            emit available(id, dest);
        }, Qt::QueuedConnection);
        return true;
    }

    if (m_downloads.contains(id)) return true;  // already in flight
    if (it->url.isEmpty()) {
        QMetaObject::invokeMethod(this, [this, id]() {
            emit downloadFailed(id,
                tr("No download URL registered for this model. "
    ...
    connect(dl, &ModelDownloader::progress, this,
            [this, id](qint64 rec, qint64 total) {
                emit downloadProgress(id, rec, total);
            });
    connect(dl, &ModelDownloader::finished, this,
            [this, id, dl](const QString& path) {
                m_downloads.remove(id);
                dl->deleteLater();
                emit available(id, path);
            });
    connect(dl, &ModelDownloader::failed, this,
            [this, id, dl](const QString& message) {
                m_downloads.remove(id);
                dl->deleteLater();
...
bool ModelRegistry::ensureAvailableSync(ModelId id, int timeoutMs) {
    ...
    connect(this, &ModelRegistry::available, &loop,
            [&](ModelId gotId, const QString&) {
                if (gotId == id) { ok = true; loop.quit(); }
            });
    connect(this, &ModelRegistry::downloadFailed, &loop,
            [&](ModelId gotId, const QString&) {
                if (gotId == id) { ok = false; loop.quit(); }
            });
    connect(&timeout, &QTimer::timeout, &loop, [&] { ok = false; loop.quit(); });
    if (!ensureAvailable(id)) return false;
