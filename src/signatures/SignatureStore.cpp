...
std::vector<Signature> SignatureStore::loadAll() const {
    ...
    const QFileInfoList entries =
        dir.entryInfoList({QStringLiteral("*.png")},
    ...
    for (const QFileInfo& fi : entries) {
        ...
        if (!s.createdAt.isValid()) s.createdAt = fi.lastModified();
...
Signature SignatureStore::add(const QImage& image,
                              const QString& label,
