...
OcrResultsDialog::OcrResultsDialog(const QString &sourceName,
                                   const QVector<OcrEngine::TextBlock> &blocks, QWidget *parent)
    : QDialog(parent) {
    ...
    if (blocks.isEmpty()) {
        ...
        m_summary->setText(tr("Found %n text region(s). Review, copy, or export below.", nullptr,
    ...
    auto *copyBtn = buttons->addButton(tr("Copy All"), QDialogButtonBox::ActionRole);
    auto *saveBtn = buttons->addButton(tr("Save as TXT…"), QDialogButtonBox::ActionRole);
...
void OcrResultsDialog::onSaveAs() {
    const QString suggested = m_suggestedBaseName + QStringLiteral(".txt");
    const QString path = QFileDialog::getSaveFileName(this, tr("Save Recognized Text"), suggested,
