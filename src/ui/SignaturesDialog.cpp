...
SignaturesDialog::SignaturesDialog(QWidget* parent) : QDialog(parent) {
    ...
    connect(m_addButton, &QPushButton::clicked,
            this, &SignaturesDialog::onAddClicked);
    connect(m_removeButton, &QPushButton::clicked,
            this, &SignaturesDialog::onRemoveClicked);
    connect(m_list, &QListWidget::itemSelectionChanged,
            this, &SignaturesDialog::onSelectionChanged);
...
QListWidgetItem* SignaturesDialog::itemForId(const QString& id) const {
    for (int i = 0; i < m_list->count(); ++i) {
        ...
        if (it->data(Qt::UserRole).toString() == id) return it;
    }
    return nullptr;
}
...
void SignaturesDialog::onAddClicked() {
    ...
    if (added.id.isEmpty()) {
        QMessageBox::warning(this, tr("Manage Signatures"),
            tr("Could not save the signature."));
        return;
    }
    reload();
