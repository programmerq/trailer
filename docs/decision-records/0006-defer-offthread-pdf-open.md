...
> (~2.5s) and stays fully responsive (thousands of event-loop ticks) while the
> **~15s isolated** annotation sweep runs on the worker; the batched GUI commit
> of all 1M annotations costs ~125–200ms (a single coalesced refresh). What
> remains synchronous on the GUI thread at open on this fixture is only the
> residual below: `QPdfDocument::load` (~1.8s) + the forms parse (~0.55s).
>
...
> not the steady state: the **true overlap is usually much narrower because
> the GUI `m_editor` is itself lazy** — its qpdf parse only fires on demand
> (the `supportsFormFilling()` AcroForm probe), and it is essentially never
> resident in a fully-swept 1M-annotation state at the same moment as the
> worker's throwaway qpdf, so the real peak sits near the single-live ~7.7 GB.
> Running two sweeps concurrently also *slows* each one — the sweep measures
> **~41s under concurrent memory pressure vs ~15s isolated** — reinforcing
> that the design keeps the two qpdf lifetimes from overlapping in practice.
>
> The full off-thread open + placeholder first page + a
> `capabilitiesChanged`-style refresh signal remains the P2 follow-up
