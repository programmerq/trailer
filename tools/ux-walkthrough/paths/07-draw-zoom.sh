...
#
# The offset is CLIENT-relative: xdotool getwindowgeometry on the Qt window id
# (from `xdotool search --class trailer`) reports the client-area origin under
# the canonical Xvfb+openbox harness session, so DY is measured from the top of
# the menu bar, NOT the WM title bar. DX/DY were re-tuned against this branch's
# build in that exact session by sweeping the toolbar and confirming a freehand
# squiggle (multi-point path, not a straight Line/Arrow) rendered under the
# cursor and dirtied the title.
#
...
# Freehand ("~") sits ~156 px in and ~48 px down from the client-area origin at
# the default 18-px icon size (menu bar ~20 px + main toolbar ~34 px stack above
# the markup row). Verified on this branch's build in the Xvfb+openbox harness;
# see SELECTOR NOTE for the client-relative geometry caveat.
